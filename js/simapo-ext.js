...
window.loadAdminSimapoMaster = async function(force = false) {
  ...
  if (data) window._allMasterData = data;
    ...
    window._allMasterData = [];
    el.innerHTML = `<div style="text-align:center;padding:30px;color:var(--danger);font-size:12px">🔌 Gagal memuat data aset. Pastikan server n8n aktif.</div>`;
  }
...
window.deleteSimapoMaster = async function(id) {
  ...
  } catch (e) { showToast('Gagal hapus aset: ' + (e?.message || e), 'error'); }
};
...
window.saveSimapoMaster = async function() {
  ...
  } catch (e) { showToast('Gagal simpan aset: ' + (e?.message || 'network error'), 'error'); }
};
...
window.submitStokOpname = async function() {
  ...
  } catch (e) { showToast('Gagal simpan opname: ' + (e?.message || 'network error'), 'error'); }
};
