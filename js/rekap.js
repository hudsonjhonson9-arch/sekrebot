...
function renderRekap(pg) {
  ...
  el.innerHTML = filteredPg.map((p, idx) => {
    ...
    return `
        <div class="rekap-card-v2" style="cursor:pointer" onclick="this.querySelector('.lokasi-detail').style.display = this.querySelector('.lokasi-detail').style.display==='none'?'block':'none';">
          <!-- TOP: Profile & Badge -->
          ...
          <!-- DETAIL: Admin-only expandable -->
          <div class="lokasi-detail" onclick="event.stopPropagation()" style="display:none; margin-top:10px; padding-top:10px; border-top:1px dashed rgba(255,255,255,0.1); font-size:10px; color:var(--muted); line-height:1.5;">
            <div style="font-weight:700;color:var(--gold);margin-bottom:6px;">📋 Detail Rekap Periode</div>
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:4px;">
              <div>👤 Nama: <strong style="color:var(--white)">${nama}</strong></div>
              <div>📌 NIP: <strong style="color:var(--white)">${nip}</strong></div>
              <div>💼 Jabatan: <span style="color:var(--white)">${jabatan || '—'}</span></div>
              <div>🏢 Bidang: <span style="color:var(--white)">${p.bidang || p.Bidang || '—'}</span></div>
              <div>📊 Total Catatan: <strong style="color:var(--gold)">${totalEntries}</strong></div>
              <div>⏳ Total Jam Hadir: <strong style="color:var(--success)">${parseFloat(p.jamHadir || 0).toFixed(1)} jam</strong></div>
              <div>🎯 Disiplin: <strong style="color:${dColor}">${dPct}%</strong></div>
              <div>📈 Kehadiran: <strong style="color:${hadirColor}">${hadirPct}%</strong></div>
            </div>
            ${isAdmin ? `<div style="margin-top:8px;padding-top:8px;border-top:1px solid rgba(255,255,255,.05);font-size:9px;color:var(--muted);font-style:italic;">💡 Klik card di mode Harian untuk edit log detail</div>` : ''}
          </div>
        </div>`;
  }).join('');
}
  ...
  try {
    ...
    const rowsExcel = lastRekapPegawai.map((p, i) => {
      ...
      if (isHarian) {
        ...
        data['Masuk Tepat Waktu'] = p.masuk || 0;
        data['Terlambat (menit)'] = _mT || '—';
        data['Pulang Tepat Waktu'] = p.pulang || 0;
        data['Pulang Cepat (menit)'] = _mC || '—';
        data['TB'] = p.alpa || '—';
        data['Izin'] = p.izin || '—';
        data['Sakit'] = p.sakit || '—';
        data['Tugas'] = p.tugas || '—';
        data['Akumulasi Keseluruhan'] = toHHMM(_mT + _mC);
      }
      return data;
    });
