/* ════ PROFESSIONAL PDF REKAP ════ */
    ...
    // ponytail: Google Drive has no CORS headers — route via weserv.nl image proxy.
    // NOTE: Google retired automated/non-browser access to `uc?export=view` in Jan 2024
    // (third-party cookie deprecation) — that path now returns 403 for bots/proxies like
    // weserv.nl. We use the `thumbnail` endpoint instead, which still serves publicly
    // shared files to automated requests. We try a couple of fallback endpoints in case
    // one gets rate-limited, and we NEVER resolve a raw (non-data:) URL — jsPDF's
    // addImage cannot fetch a network URL on the fly, so a raw-URL fallback silently
    // fails to render. If every attempt fails, we resolve null so the caller can skip
    // drawing the signature (and log which NIP/file failed for debugging).
    async function _fetchSigToBase64(url, label) {
      ...
      if (!fileId) {
        console.warn(`[sig] Gagal ekstrak fileId dari URL${label ? ' (' + label + ')' : ''}:`, url);
        return null;
      }

      const candidateUrls = [
        `https://images.weserv.nl/?url=${encodeURIComponent(`https://drive.google.com/thumbnail?id=${fileId}&sz=w1000`)}`,
        `https://drive.google.com/thumbnail?id=${fileId}&sz=w1000`,
        `https://images.weserv.nl/?url=${encodeURIComponent(`https://lh3.googleusercontent.com/d/${fileId}=w1000`)}`
      ];

      const tryLoad = (srcUrl) => new Promise((resolve) => {
        const img = new Image();
        img.crossOrigin = 'Anonymous';
        img.onload = () => {
          try {
      ...
      console.warn(`[sig] Semua endpoint gagal memuat tanda tangan${label ? ' (' + label + ')' : ''} — fileId: ${fileId}`);
      return null;
    }
      ...
      try {
        ...
        try {
          ...
          if (Array.isArray(sigData)) {
            const sigEntries = sigData.filter(s => s.signature && s.signature.length > 10);
            const sigPromises = sigEntries.map(s => _fetchSigToBase64(s.signature, s.nip || s.telegram_id || s.id));
            const sigResults = await Promise.all(sigPromises);
            sigEntries.forEach((s, i) => {
              const resolved = sigResults[i];
              if (!resolved) return; // gagal dimuat — skip, jangan simpan URL mentah
              if (s.nip) sigMap[String(s.nip)] = resolved;
              if (s.telegram_id) sigMap[String(s.telegram_id)] = resolved;
            });
          }
        } catch (e) {
          console.warn("Gagal load paraf, lanjut tanpa paraf:", e);
        }
            ...
            if (data.cell.section === 'body' && (data.column.index === 4 || data.column.index === 6)) {
              ...
              if (sig) {
                ...
                if (jamVal && !['—', '-', 'Alpa', 'Alpha', 'Tanpa Berita', 'TB', 'Belum'].includes(jamVal.trim())) {
                  try {
                    const sigFmt = sig.startsWith('data:image/jpeg') ? 'JPEG' : 'PNG';
                    doc.addImage(sig, sigFmt, data.cell.x + 2, data.cell.y + 2, 18, 10, undefined, 'FAST');
                  } catch (err) { console.warn('Gagal gambar paraf:', err); }
                }
              }
            }
          },
        ...
        if (kabanSig) {
          try {
            const kabanFmt = kabanSig.startsWith('data:image/jpeg') ? 'JPEG' : 'PNG';
            doc.addImage(kabanSig, kabanFmt, signatureX - 15, footerY + 14, 30, 15, undefined, 'FAST');
          } catch (e) { console.warn('Gagal gambar tanda tangan Kepala:', e); }
        }
          ...
          (async () => {
            try {
              ...
              if (Filesystem) {
                ...
                // ponytail: doc.save() triggers a download manager — use blob URL instead
                const blob = window.lastGeneratedDoc.output('blob');
                const url = URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url; a.download = window.lastGeneratedFileName;
