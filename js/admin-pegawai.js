        ...
        if (!saveOk || (d && d.ok === false)) {
          ...
          try {
            if (cleanRole === 'admin' || cleanRole === 'superadmin') {
              await apiPost(P.adminAdd, { telegram_id: Number(id), nip, nama, role: cleanRole, ditambahkan_oleh: Number(MY_ID) });
            } else if (isEdit) {
              // ponytail: downgrade admin → user: remove admin record
              await apiPost(P.adminDel, { telegram_id: Number(id) });
            }
          } catch (err) {
            console.warn('Admin role sync failed:', err);
            showResult('pegawaiFormResult', 'pegawaiFormRIcon', 'pegawaiFormRTitle', 'pegawaiFormRMsg', 'warning', '⚠️', 'Peringatan', 'Data tersimpan, tapi gagal sync role admin. Hubungi admin server.');
            dom.show('pegawaiFormResult');
          }
